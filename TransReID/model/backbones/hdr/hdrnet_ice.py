""" bilateral filter network in PyTorch

Acknowledgments:
* Coeffs提取边缘特征x, 生成bilateral grid y。
* 网络只更新Coeffs中部分，不更新生成guidance的部分

by / Copyright 2024, ZWQ
"""
...
class Slice(nn.Module):
    ...
    def forward(self, bilateral_grid, guidemap): 
        ...
        return coeefs # batch*12*256*128
        # Nx12x8x16x16
...
class Coeffs(nn.Module):
    ...
    def __init__(self, nin=4, nout=3, params=None):
        ...
        nsize = params.HDRNET.NET_INPUT_SIZE
        h, w = nsize
        ...
        # splat features
        n_layers_splat = int(np.log2(h/sb))
        self.splat_features = nn.ModuleList()
        prev_ch = 3
        for i in range(n_layers_splat):
            use_bn = bn if i > 0 else False
            self.splat_features.append(ConvBlock(prev_ch, cm*(2**i)*lb, 3, stride=2, batch_norm=use_bn)) # channel->*8*(2^i)
    ...
    def forward(self, lowres_input):
        ...
        fusion_grid = local_features # b,64, 16, 16
        fusion_global = global_features.view(bs,8*cm*lb,1,1) # b,64, 1, 1

        fusion = self.relu( fusion_grid + fusion_global ) # b,64, 16, 16

        x = self.conv_out(fusion) # b, 96, 16, 16
        s = x.shape # 8,96,16,16
...
class HDRPointwiseNN(nn.Module):
    ...
    def forward(self, lowres, fullres=None):
        edge_feat, coeffs = self.coeffs(lowres)  # low resolution  
        guide = self.guide(fullres) # full resolusion
        slice_coeffs = self.slice(coeffs, guide) # batch*1*h*w
        out = self.apply_coeffs(slice_coeffs, fullres)
        # out = bsa.bsa(coeffs,guide,fullres)
        return out, edge_feat

    def load_param(self, hdr_path):
        param_dict = torch.load(hdr_path)
        self.state_dict(param_dict)
        print('load HDRNet from {}'.format(hdr_path))
#########################################################################################################
