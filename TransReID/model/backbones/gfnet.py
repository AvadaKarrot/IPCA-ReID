...
class GlobalFilter(nn.Module):
    ...
    def forward(self, x, spatial_size=(16,8)):
        B, N, C = x.shape
        if spatial_size is None:
            a = b = int(math.sqrt(N))
        else:
            a, b = spatial_size
        # 检查patches的总数是否与a * b相符
        assert a * b == N, "spatial_size does not match number of patches"
        x = x.view(B, a, b, C) # ([128, 16, 8, 512])

        x = x.to(torch.float32)

        x = torch.fft.rfft2(x, dim=(1, 2), norm='ortho') # ([128, 16, 5, 512])
        weight = torch.view_as_complex(self.complex_weight)
        x = x * weight # e([128, 16, 5, 512])
        x = torch.fft.irfft2(x, s=(a, b), dim=(1, 2), norm='ortho') # [128, 16, 8, 512])
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768):
        ...
        num_patches = (img_size[1] // patch_size[1]) * (img_size[0] // patch_size[0]) # 64 * 32
        self.img_size = img_size # 256,128
        self.patch_size = patch_size # 4,4
        self.num_patches = num_patches # 64 * 32
    ...
    def load_param(self, model_path):
        ...
        for k, v in param_dict.items():
            ...
            if 'complex_weight' in k:
                h, w, dim = v.shape[0:3]
                new_h, new_w, new_dim = self.blocks[0].filter.complex_weight.shape[0:3]
                weight = v.reshape(1,h,w,dim*2).permute(0,3,1,2)
                v = torch.nn.functional.interpolate(
                    weight, size=(new_h, new_w), mode='bicubic', align_corners=True).permute(0,2,3,1).reshape(new_h, new_w, new_dim, 2)
