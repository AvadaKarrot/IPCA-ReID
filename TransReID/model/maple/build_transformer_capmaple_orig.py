...
class build_transformer_caption(nn.Module):
    def __init__(self, num_classes, camera_num, cfg, factory):
        ...
        ###############################zwq
        self.caption = cfg.MODEL.CAPTION
    ...
    def caption_features(self, captions):
        ...
        if self.coop:
            ...
            # all_text_features.append(text_features_ele)
    ...
    def forward(self, x, label=None, captions=None,cam_label= None, low = None, full=None, cal_covstat=False):
        '''
...
######################################################################################### 
            if self.caption:
                return cls_score, global_feat, logits
